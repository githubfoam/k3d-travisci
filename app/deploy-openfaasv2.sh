...
# Create a password for the OpenFaaS Gateway
# add it as a secret into the cluster.
...
# The YAML installation is only recommended for local development
# for deployment into a production environment it is recommended you use the Helm installation
kubectl apply -f ./yaml

# Install the faas-cli
# brew install faas-cli # METHOD1
# curl -sSL https://cli.openfaas.com | sudo sh # METHOD2
curl -sSL https://cli.openfaas.com | sh # METHOD2


# Configure faas-cli & Login to the OpenFaaS Dashboard
# Configure the faas-cli to use your local OpenFaaS cluster by using the faas-cli login command
# If running k3d you’ll need to forward the gateway service port and set the OPENFAAS_URL environment variable
kubectl port-forward svc/gateway -n openfaas 31112:8080 &
export OPENFAAS_URL=http://127.0.0.1:31112
echo $PASSWORD | faas-cli login --password-stdin


# Open a browser and navigate to http://localhost:31112 to load the UI.
# The UI will prompt for a username and password.
# The default username is admin
# the password is the one you specified in the deployment instructions above
